...
# parameters
p = {k: v.numpy() for k, v in GPT2Model.from_pretrained(args.model).state_dict().items()}
...
def mlp(x, i):
  w, b = p[f"h.{i}.mlp.c_fc.weight"], p[f"h.{i}.mlp.c_fc.bias"]
  x = gelu(linear(x, w, b))
  w, b = p[f"h.{i}.mlp.c_proj.weight"], p[f"h.{i}.mlp.c_proj.bias"]
  return linear(x, w, b)
...
def mha(x, i):
  T, C = x.shape
  w, b = p[f"h.{i}.attn.c_attn.weight"], p[f"h.{i}.attn.c_attn.bias"]
  x = linear(x, w, b)
  ...
  w, b = p[f"h.{i}.attn.c_proj.weight"], p[f"h.{i}.attn.c_proj.bias"]
  x = linear(x, w, b)
  return x

def transformer_block(x, i):
  w, b = p[f"h.{i}.ln_1.weight"], p[f"h.{i}.ln_1.bias"]
  x = x + mha(layer_norm(x, w, b), i)
  w, b = p[f"h.{i}.ln_2.weight"], p[f"h.{i}.ln_2.bias"]
  x = x + mlp(layer_norm(x, w, b), i)
  return x

def gpt2(ids):
  wte, wpe = p["wte.weight"], p["wpe.weight"]
  x = wte[ids] + wpe[range(len(ids))]
  for i in range(hparams["n_layer"]):
    x = transformer_block(x, i)
  w, b = p["ln_f.weight"], p["ln_f.bias"]
  x = layer_norm(x, w, b)
  logits = x[-1, :] @ wte.T
